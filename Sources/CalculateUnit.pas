...
begin
  Expression := StringReplace(Expression, ' ', '', [rfReplaceAll]); // удаляем все пробелы
  ...
  begin
    ...
    begin
      SubExpression := Copy(Expression, Brackets.OpeningBracketPos + 1, Brackets.ClosingBracketPos - Brackets.OpeningBracketPos - 1); // получаем выражение, находящееся между скобок
    ...
    if ExpressionResult = 'error' then // если выражение не было посчитано - прерываем цикл
      Break;
    Expression := StuffString(Expression, Brackets.OpeningBracketPos, Length(SubExpression) + 2, ExpressionResult); // заменяем в исходном выражении блок со скобками на результат вычисления этого блока

    Brackets := GetFirstPairedBrackets(Expression);
  end;

  // вычисляем выражение, оставшееся после расрытия скобок
...
begin
  ...
  begin
    ...
    OpeningBracketPos := Length(Expression) - PosEx(OpeningBrackets[BracketIndex], Expression, Length(Expression) - ClosingBracketPos + 1) + 1; // в перевёрнутой строке находим ближаюшую закрывающую к уже найденной скобке и вычисляем её позицию в исходной строке
  end
  else
  begin
    OpeningBracketPos := 0;
